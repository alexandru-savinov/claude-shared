...
nix eval .#nixosConfigurations.<host>.config.<option>   # inspect current value
nix repl .#  # interactive flake exploration
nixos-rebuild dry-build --flake .#<host> 2>&1 | grep -iE "warn|error"  # capture current warnings/errors
```
...
```bash
nixos-rebuild dry-build --flake .#<host> 2>&1 | grep -iE "warn|error"
```
...
- [ ] Run `nixos-rebuild dry-build --flake .#<host>` and capture current warnings/errors
- [ ] Confirm which module sets the option causing the issue (use `grep -r` or `nix eval`)
- [ ] Verify the option exists: `nix eval .#nixosConfigurations.<host>.config.<option>` (eval error = option does not exist)
- [ ] After the change, re-run `nixos-rebuild dry-build --flake .#<host>` and diff the output

---

## Example (Good)

> **Hypothesis:** The Home Manager warning about `useGlobalPkgs` is caused by the option being explicitly set to `true` in `hosts/<host-a>/configuration.nix`, which triggers a warning in this version of Home Manager.
>
> **Test:** `grep -r "useGlobalPkgs" hosts/`
>
> **Output:**
> ```
> hosts/<host-a>/configuration.nix:58:    useGlobalPkgs = true;
> hosts/<host-b>/configuration.nix:60:    useGlobalPkgs = true;
> ```
>
> **Confirmed.** Both hosts set it; the `<host-b>` instance on line 60 is the one triggering the warning.
>
> **Fix:** Remove line 60.
>
> **Verify:** Re-run `nixos-rebuild dry-build --flake .#<host>`. Warning is gone.
